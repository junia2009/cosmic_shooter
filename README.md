...
> **Ver.1.6.1** | [プレイする（GitHub Pages）](https://junia2009.github.io/cosmic_shooter/)
...
- **モバイル最適化（v1.6.0）** — 専用FIREボタン・カメラ実FOVに基づく正確な操作感度・移動と射撃を別の指で同時操作・iPhoneセーフエリア対応
- **Service Worker による自動アップデート（v1.6.1）** — 新バージョン公開時、次回起動で自動反映（10分のHTTPキャッシュ待ちなし）
...
| 操作 | PC | スマホ |
|------|-----|--------|
| 自機移動 | マウスドラッグ / 矢印キー | 画面ドラッグ |
| ショット | クリック / スペースキー（長押しで連射） | **FIREボタン長押しで連射** |

スマホでは画面右下の **FIRE** ボタンを長押ししている間ずっと連射されます。  
**移動指と射撃指は別で同時操作可能**（マルチタッチ対応）。
...
sw.js               Service Worker（自動アップデート + オフライン対応）
